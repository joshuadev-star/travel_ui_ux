import React from "react";
import Button from "./Button";
import Image from "next/image";

const GetApp = () => {
  return (
    <section className="flexCenter w-full flex-col px-4 pb-16 sm:px-6 sm:pb-[100px]">

      <div className="get-app">

        {/* TEXT */}
        <div className="z-20 flex w-full flex-1 flex-col items-start justify-center gap-8 sm:gap-10">

          <h2 className="bold-40 lg:bold-64 xl:max-w-[320px]">
            Get for free now!
          </h2>

          <p className="regular-16 text-gray-10">
            Available on iOS and Android
          </p>

          <div className="flex w-full max-w-[400px] flex-col gap-3 sm:flex-row">

            <Button
              type="button"
              title="App Store"
              icon="/apple.svg"
              variant="btn_white"
              full
            />

            <Button
              type="button"
              title="Play Store"
              icon="/android.svg"
              variant="btn_dark_green_outline"
              full
            />

          </div>
        </div>

        {/* PHONES */}
        <div className="flex w-full flex-1 items-center justify-center sm:justify-end">
          <Image
            src="/phones.png"
            alt="phones"
            width={550}
            height={870}
            className="h-auto w-full max-w-[550px] object-contain"
          />
        </div>

      </div>
    </section>
  );
};

export default GetApp;